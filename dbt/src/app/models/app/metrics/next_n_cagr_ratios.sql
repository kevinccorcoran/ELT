...
WITH base AS (
    -- Select base data excluding rows where n is zero
    SELECT
        ticker,
        n,
        cagr,
...
next_n AS (
    -- Calculate the next value of n using LEAD, defaulting to 54 if no next value exists
    SELECT
        n,
...
joined_data AS (
    -- Join base data with the next_n values and retrieve corresponding next CAGR and group
    SELECT
        b.ticker,
        b.n,
        nn.next_n,
    ...
    FROM
        base b
    JOIN
        next_n nn ON b.n = nn.n
    INNER JOIN
        base ijn ON b.ticker = ijn.ticker
        AND nn.next_n = ijn.n
        AND b."type" = ijn."type"
),

grouped_data AS (
    -- Group data by n, next_n, cagr_group, next_cagr_group, and type, and count occurrences
    SELECT
        n,
        next_n,
        cagr_group,
